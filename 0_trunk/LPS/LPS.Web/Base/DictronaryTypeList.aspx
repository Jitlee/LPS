<%@ Page Title="" Language="C#" MasterPageFile="~/Main/MasterPage.master" AutoEventWireup="true"
	CodeBehind="DictronaryTypeList.aspx.cs" Inherits="LPS.Web.Base.BaseDictronaryTypeList" %>
	...
	<script type="text/javascript">
		...
		$(document).ready(function () {
			$(".headerBtnAdd").click(function () {
				var murl = 'DictronaryTypeEdit.aspx';
			...
			editArr.each(function (i, o) {
				var obj = $(o);
				obj.click(function () {
					var murl = 'DictronaryTypeEdit.aspx?opType=alert&id=' + obj.attr("Guid");
					diag = new Dialog("Edit");
					diag.URL = murl;
					diag.Title = "修改";
					diag.Width = 600;
							...
							<asp:GridView ID="gvBaseDictronaryType" AutoGenerateColumns="false" runat="server">
								<Columns>
									...
									<asp:TemplateField HeaderText="编辑" HeaderStyle-Width="4%">
										<ItemTemplate>
											<a guid="<%# Eval("DictType") %>" class="gvEdit">
									...
									<asp:TemplateField HeaderText="删除" HeaderStyle-Width="4%">
										<ItemTemplate>
											<asp:ImageButton ID="ibtn_delete" CssClass="deleteTS" CommandName="delete" OnCommand="GView_LinkButton_Click"
												CommandArgument='<%#Eval("DictType") %>' ImageUrl="~/images/Common/delete.gif"
												runat="server" />
										</ItemTemplate>
									</asp:TemplateField>
