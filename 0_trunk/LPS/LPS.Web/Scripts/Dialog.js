 ...
 * Version 2.1 beta (2012-6-10)
 * @requires jQuery v1.2.6 or later
 *
 * Email: ritacc@126.com
 * Copyright (c) 2009-2012 朱长双
 * 
 * 浏览器兼容性
...
Dialog.prototype.show = function () {
    window.top.$.DialogLayers.push(this);
    ...
    arr.push("            </div></td></tr>");
    arr.push("      </table><a href='#;' onfocus='$MP(\"_forTab_" + this.ID + "\").focus();'></a></td>");
    ...
    if (!bgdiv) {
        ...
        if (isIE6) {
            ...
            var bgIframe = pw.document.createElement('<iframe id="frm" src="about:blank" style="filter:alpha(opacity=1);" width="100%" height="100%"></iframe>');
    ...
    var div = pw.$MP("_DialogDiv_" + this.ID);
    if (!div) {
    ...
    if (this.innerElementId) {
        var innerElement = $MP(this.innerElementId);
        ...
        if (isIE) {
            ...
            pw.$MP("_Covering_" + this.ID).parentNode.appendChild(fragment)
        } else {
            pw.$MP("_Covering_" + this.ID).parentNode.appendChild(innerElement)
        }
    }
    pw.$MP("_DialogDiv_" + this.ID).DialogInstance = this;
    if (this.URL)
        pw.$MP("_DialogFrame_" + this.ID).DialogInstance = this;
    pw.Drag.init(pw.$MP("_draghandle_" + this.ID), pw.$MP("_DialogDiv_" + this.ID)); //注册拖拽方法
    if (!isIE) {
        pw.$MP("_DialogDiv_" + this.ID).dialogId = this.ID;
        pw.$MP("_DialogDiv_" + this.ID).onDragStart = function () { $MP("_Covering_" + this.dialogId).style.display = "" }
        pw.$MP("_DialogDiv_" + this.ID).onDragEnd = function () { $MP("_Covering_" + this.dialogId).style.display = "none" }
    }

    this.OKButton = pw.$MP("_ButtonOK_" + this.ID);
    this.CancelButton = pw.$MP("_ButtonCancel_" + this.ID);

    //显示标题图片
    if (this.ShowMessageRow) {
        $E.show(pw.$MP("_MessageRow_" + this.ID));
        if (this.MessageTitle) {
            pw.$MP("_MessageTitle_" + this.ID).innerHTML = this.MessageTitle;
        }
        if (this.Message) {
            pw.$MP("_Message_" + this.ID).innerHTML = this.Message;
        }
    }

    //显示按钮栏
    if (!this.ShowButtonRow) {
        pw.$MP("_ButtonRow_" + this.ID).hide();
    }
    if (this.CancelEvent) {
...
Dialog.prototype.close = function (str) {
    if (this.innerElementId) {
        var innerElement = $E.getTopLevelWindow().$MP(this.innerElementId);
    ...
    if (this.WindowFlag) {
        ...
        if (this.AlertFlag) {
            $E.hide(pw.$MP("_AlertBGDiv"));
        }
        if (!flag && !this.AlertFlag) {//此处是为处理弹出窗口被关闭后iframe立即被重定向时背景层不消失的问题
            pw.eval('window._OpacityFunc = function(){var w = $E.getTopLevelWindow();$E.hide(w.$MP("_DialogBGDiv"));}');
            pw._OpacityFunc();
        }
...
Dialog.prototype.addButton = function (id, txt, func) {
    ...
    var pw = $E.getTopLevelWindow();
    pw.$MP("_DialogButtons_" + this.ID).$T("input")[0].getParent("a").insertAdjacentHTML("beforeBegin", html);
    pw.$MP("_Button_" + this.ID + "_" + id).onclick = func;
}
...
Dialog.alert = function (msg, func, w, h) {
    ...
    diag.show();
    pw.$MP("_AlertBGDiv").style.display = "";
    $E.hide(pw.$MP("_ButtonOK_" + diag.ID));
    var win = pw.$MP("_DialogFrame_" + diag.ID).contentWindow;
    ...
    if (diag.ShowButtonRow)
        diag.CancelButton.focus();
    pw.$MP("_DialogButtons_" + diag.ID).style.textAlign = "center";
}
...
Dialog.confirm = function (msg, func1, func2, w, h) {
    ...
    diag.show();
    pw.$MP("_AlertBGDiv").style.dispaly = "";
    var win = pw.$MP("_DialogFrame_" + diag.ID).contentWindow;
    var doc = win.document;
    doc.open();
    doc.write("<body oncontextmenu='return false;'></body>");
    ...
    if (d) {
        try {
            ...
            if (!this.AlertFlag) {
                ...
                if (!flag) {
                    // $E.getTopLevelWindow().$MP("_DialogBGDiv").style.opacity = "0.4";
                    // $E.getTopLevelWindow().$MP("_DialogBGDiv").style.filter = "alpha(opacity=40)";
                }
            }
            if (d.AlertFlag) {
                $E.show($E.getTopLevelWindow().$MP("_AlertBGDiv"));
            }
...
Dialog.setPosition = function () {
    ...
    for (i = 0; i < DialogArr.length; i++) {
        try {
            pw.$MP("_DialogDiv_" + DialogArr[i]).DialogInstance.setPosition();
        } catch (ex) { }
    }
}
Dialog.prototype.setPosition = function () {
    ...
    pw.$MP(this.bgdivID).style.height = sh + "px";
}
...
; (function ($) {
    ...
    $.DialogClose = function (va) {

        var formCount = window.top.$.DialogLayers.length;
        if (formCount > 0) {
