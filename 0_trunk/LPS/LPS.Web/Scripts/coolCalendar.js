...
document.writeln('<iframe id=meizzDateLayer Author=wayx frameborder=0 style="position: absolute; width: 50; height: 900; z-index: 9998; display: none;overflow-y:none;"></iframe>');
strFrame='<style>';
...
strFrame+='</table></td></tr><!-- Site:FLYSO Blog http://www.flyso.net -->';
...
for (i=35;i<39;i++)strFrame+='<td width=20 height=20 id=meizzDay'+i+' style="font-size:12px;color:#900;" Author=wayx onclick="parent.meizzDayClick(this.innerText,0)"></td>';
...
var outTime = "";
function setday(tt,obj,isTime) //主调函数
{
 if (arguments.length >  3){alert("对不起!传入本控件的参数太多!");return;}
 if (arguments.length == 0){alert("对不起!您没有传回本控件任何参数!");return;}
 var date = new Date();
 if(isTime == true){ outTime= " " + date.getHours() + ":" + date.getMinutes();}else {outTime = "";}
...
function meizzPrevY() //往前翻年
{
    if(meizzTheYear > 999 && meizzTheYear <10000){meizzTheYear--;}
    else{alert("年份超出范围（1000-9999）!");}
    meizzSetDay(meizzTheYear,meizzTheMonth);
}
function meizzNextY() //往后翻年
{
    if(meizzTheYear > 999 && meizzTheYear <10000){meizzTheYear++;}
    else{alert("年份超出范围（1000-9999）!");}
    meizzSetDay(meizzTheYear,meizzTheMonth);
}
function meizzToday() //今天
{
 var today;
...
function meizzSetDay(yy,mm) //程序的主要部分**********
{
 ...
 if(mm<1){
  ...
  if (outObject)
  {
    if (!n) {
      return;}
    if ( n < 10){n = "0" + n;}
    outObject.value= yy + "-" + mm + "-" + n + outTime; //注：在这里你可以输出改成你想要的格式
    closeLayer(); 
    var oTip = document.getElementById(outObject.id + "_tip");
    if(oTip) {
        oTip.click();
